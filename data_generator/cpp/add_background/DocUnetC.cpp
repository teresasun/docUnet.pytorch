...
Mat folds_curves_img_vector(const Mat img, float vec_rows[], float vec_cols[], const int new_img_row, const int new_img_col, const int n, int index)
{
    ...
    int cols = img.cols;
    int relax = 200 * 4 * n;
    Mat new_img = Mat(rows + relax, cols + relax, img.type(), Scalar(150, 180, 180));
...
Mat linear_interp(Mat img, int field_size)
{
    ...
    for (int row_i = abs(start); row_i < rows - end; row_i++)
    {
        for (int col_j = abs(start); col_j < cols - end; col_j++)
        {
            ...
            if (temp_pixel[0] == 150 && temp_pixel[1] == 180 && temp_pixel[2] == 180)
            {
                int num, a, b, c;
                num = a = b = c = 0;
                Vec3b temp_pixel;
...
void warp_images(Mat new_img,const Mat img,float vec_rows[], float vec_cols[],const int index)
{
    ...
    // 对图像进行透视变换
	warpPerspective(new_img, new_img, M, cv::Size(cols,rows), cv::INTER_LINEAR,BORDER_CONSTANT,cv::Scalar(150,180,180));
...
Mat add_background(Mat img, const string &background_path, int board)
{
    ...
    for (int col_j = 0; col_j < cols; col_j++)
    {
        for (int row_i = 0; row_i < rows; row_i++)
        {
            ...
            {
//                if (temp_pixel[0] > 40 || temp_pixel[1] < 200 || temp_pixel[2] > 40)
                if (temp_pixel[0] == 150 && temp_pixel[1] == 180 && temp_pixel[2] == 180)
                {
                    continue;//background_image.at<Vec3b>(row_i + row_start, col_j + col_start) = temp_pixel;
                }
                else
                {
                    background_image.at<Vec3b>(row_i + row_start, col_j + col_start) = temp_pixel;
...
// Parameter: float vec_cols[] 列方向的映射矩阵,和,vec_rows结合使用, vec_rows[x] = x1,vec_rows[y] = y1 表示扫描图上(x,y)处的点在扭曲图上(x1,y1)处取值
